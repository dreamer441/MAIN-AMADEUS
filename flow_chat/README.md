...
Layer 1 contains only `chat_id`, title, and description. It never loads or injects dedicated-chat message bodies. The metadata is read from the current dedicated-chat store for each Flow context build, so created, renamed or otherwise metadata-updated, and deleted chats are reflected without Flow maintaining a copy.
